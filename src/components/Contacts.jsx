...
export default function Contacts({ session, contacts, reload }) {
  ...
  async function save() {
    ...
    setSaving(true)
    const payload = { type: form.type, name: form.name, company: form.company || null, email: form.email || null, phone: form.phone || null, address: form.address || null, notes: form.notes || null, user_id: session.user.id }
  ...
  async function del(id) {
    ...
    await supabase.from('contacts').delete().eq('id', id); reload()
  }
  ...
  return (
    <div className="pf-contacts">
      ...
      <div className="pf-type-chips">
        <button className={`pf-type-chip pf-type-chip-all ${filterType === 'all' ? 'pf-type-chip-active' : ''}`} onClick={() => setFilterType('all')}>
          <span className="pf-type-chip-count">{contacts.length}</span>
          <span className="pf-type-chip-label">All</span>
        </button>
        {CONTACT_TYPES.map(t => (
          <button key={t} className={`pf-type-chip pf-type-chip-${TYPE_TONE[t]} ${filterType === t ? 'pf-type-chip-active' : ''}`} onClick={() => setFilterType(filterType === t ? 'all' : t)}>
      ...
      <div className="pf-search-row">
        <div className="pf-search">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="pf-search-icon"><circle cx="6" cy="6" r="4" stroke="currentColor" strokeWidth="1.5"/><path d="M9 9l3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
          <input type="search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, company or email…" className="pf-search-input" />
          {search && (
        ...
        <div className="pf-search-meta">{filtered.length} of {contacts.length}</div>
      </div>

      {!filtered.length ? (
        <div className="pf-empty-card">
          <div className="pf-empty-icon">
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="8" r="4"/><path d="M4 18c0-3 3-5 7-5s7 2 7 5"/></svg>
          </div>
          ...
          {!contacts.length && <button className="pf-cta pf-cta-primary pf-cta-sm" onClick={openAdd} style={{ marginTop: 16 }}><span>+ Add your first contact</span></button>}
        </div>
      ) : (
            ...
            return (
              <article key={c.id} className={`pf-contact-card pf-card-tone-${tone}`}>
                ...
                <header className="pf-contact-head">
                  ...
                  <span className={`pf-tag pf-tag-${tone}`}><span className="pf-tag-dot" />{c.type}</span>
                </header>
                <div className="pf-contact-body">
                  {c.email && (
                    <a href={`mailto:${c.email}`} className="pf-contact-line">
                      <span className="pf-contact-icon"><svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="10" height="8" rx="1"/><path d="M2.5 4l4.5 3.5L11.5 4"/></svg></span>
                      <span className="pf-contact-text">{c.email}</span>
                    </a>
                  )}
                  {c.phone && (
                    <a href={`tel:${c.phone}`} className="pf-contact-line">
                      <span className="pf-contact-icon"><svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M3 2.5h2l1 2.5-1.2.9a7 7 0 003.3 3.3l.9-1.2L11.5 9V11a1 1 0 01-1.1 1A9 9 0 012 3.6 1 1 0 013 2.5z"/></svg></span>
                      <span className="pf-contact-text">{c.phone}</span>
                    </a>
                  )}
                  {c.address && (
                    <div className="pf-contact-line pf-contact-line-plain">
                      <span className="pf-contact-icon"><svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M7 12.5c2.5-3 4-5 4-7a4 4 0 10-8 0c0 2 1.5 4 4 7z"/><circle cx="7" cy="5.5" r="1.4"/></svg></span>
            ...
            <div className="pf-form-grid">
              <Field label="Type" full>
                <div className="pf-type-radio">
                  {CONTACT_TYPES.map(t => (
                    <button key={t} type="button" className={`pf-type-radio-btn pf-type-radio-${TYPE_TONE[t]} ${form.type === t ? 'pf-type-radio-active' : ''}`} onClick={() => setField('type', t)}>
                      <span className="pf-tag-dot" />{t}
                    </button>
                  ))}
                </div>
              </Field>
              <Field label="Name" required full>
                <input type='text' value={form.name} onChange={e => setField('name', e.target.value)} placeholder='Jane Smith' className="pf-input" autoFocus />
              </Field>
              <Field label="Company">
              ...
              <Field label="Notes" full>
                <textarea value={form.notes} onChange={e => setField('notes', e.target.value)} rows={3} placeholder='Anything to remember…' className="pf-input pf-textarea" />
              </Field>
            </div>
...
.pf-page-head { display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 16px; margin-bottom: 1.5rem; }
...
.pf-type-chips { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 1rem; }
.pf-type-chip { display: inline-flex; align-items: center; gap: 8px; background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: 999px; padding: 6px 14px 6px 8px; cursor: pointer; font-family: Georgia, serif; font-size: 13px; color: var(--pl-text-2); transition: all .18s ease; }
.pf-type-chip:hover { border-color: var(--pl-border-2); transform: translateY(-1px); box-shadow: var(--pl-shadow-1); }
.pf-type-chip-count { display: inline-flex; align-items: center; justify-content: center; min-width: 24px; padding: 0 6px; height: 22px; border-radius: 999px; font-family: ui-monospace, 'SF Mono', Menlo, monospace; font-size: 11px; font-weight: 500; background: var(--pl-bg-3); color: var(--pl-text); letter-spacing: -0.005em; }
...
${['green','blue','amber','purple','teal','red','indigo','grey'].map(t => `
.pf-type-chip-${t}.pf-type-chip-active { background: var(--t-${t}-bg); border-color: var(--t-${t}-bd); color: var(--t-${t}-fg); }
.pf-type-chip-${t}.pf-type-chip-active .pf-type-chip-count { background: var(--t-${t}-fg); color: var(--pl-bg); }
`).join('')}

.pf-search-row { display: flex; align-items: center; gap: 12px; margin-bottom: 1.25rem; flex-wrap: wrap; }
.pf-search { position: relative; display: flex; align-items: center; flex: 1; min-width: 240px; max-width: 480px; background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: 999px; padding: 0 12px; transition: border-color .18s, box-shadow .18s; }
...
.pf-search-input { flex: 1; border: 0; outline: 0; background: transparent; font-family: Georgia, serif; font-size: 13.5px; color: var(--pl-text); padding: 10px 12px; }
.pf-search-input::placeholder { color: var(--pl-muted); }
.pf-search-clear { background: var(--pl-bg-3); border: 0; border-radius: 50%; width: 20px; height: 20px; cursor: pointer; color: var(--pl-muted); display: inline-flex; align-items: center; justify-content: center; transition: color .18s, background .18s; }
.pf-search-clear:hover { color: var(--pl-text); background: var(--pl-border); }
.pf-search-meta { font-family: ui-monospace, monospace; font-size: 11px; color: var(--pl-muted); letter-spacing: .04em; }

.pf-card-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(290px, 1fr)); gap: 12px; }
.pf-contact-card { position: relative; background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: 16px; padding: 1.25rem 1.25rem 1.1rem; box-shadow: var(--pl-shadow-1); overflow: hidden; transition: transform .2s ease, box-shadow .2s ease, border-color .2s ease; display: flex; flex-direction: column; gap: .85rem; }
.pf-contact-card:hover { transform: translateY(-2px); box-shadow: var(--pl-shadow-2); }
.pf-contact-glow { position: absolute; inset: 0; background: radial-gradient(ellipse 70% 50% at 50% 0%, color-mix(in oklab, currentColor 10%, transparent), transparent 70%); pointer-events: none; opacity: 0; transition: opacity .25s ease; }
.pf-contact-card:hover .pf-contact-glow { opacity: 1; }
${['green','blue','amber','purple','teal','red','indigo','grey'].map(t => `
.pf-card-tone-${t} { color: var(--t-${t}-fg); }
.pf-card-tone-${t}:hover { border-color: var(--t-${t}-bd); }
.pf-card-tone-${t}::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, var(--t-${t}-fg), color-mix(in oklab, var(--t-${t}-fg) 60%, transparent)); opacity: 0; transition: opacity .2s; }
.pf-card-tone-${t}:hover::before { opacity: .8; }
`).join('')}

.pf-contact-head { display: flex; align-items: center; gap: 12px; position: relative; }
.pf-avatar { flex-shrink: 0; width: 44px; height: 44px; border-radius: 14px; display: inline-flex; align-items: center; justify-content: center; font-family: Georgia, serif; font-size: 16px; font-weight: 500; letter-spacing: -0.01em; border: 1px solid; position: relative; }
.pf-avatar::after { content: ''; position: absolute; inset: 0; border-radius: 14px; background: linear-gradient(135deg, rgba(255,255,255,.15), rgba(255,255,255,0) 60%); pointer-events: none; }
${['green','blue','amber','purple','teal','red','indigo','grey'].map(t => `
.pf-avatar-${t} { background: linear-gradient(135deg, color-mix(in oklab, var(--t-${t}-fg) 18%, var(--t-${t}-bg)) 0%, var(--t-${t}-bg) 100%); color: var(--t-${t}-fg); border-color: var(--t-${t}-bd); }
`).join('')}

.pf-contact-id { flex: 1; min-width: 0; color: var(--pl-text); }
.pf-contact-name { font-family: Georgia, serif; font-weight: 500; font-size: 15px; letter-spacing: -0.005em; color: var(--pl-text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.pf-contact-company { font-family: ui-monospace, monospace; font-size: 11.5px; color: var(--pl-muted); margin-top: 1px; letter-spacing: .02em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

.pf-tag { display: inline-flex; align-items: center; gap: 6px; font-family: ui-monospace, monospace; font-size: 10.5px; font-weight: 500; letter-spacing: .04em; text-transform: uppercase; padding: 3px 9px 3px 7px; border-radius: 999px; border: 1px solid transparent; white-space: nowrap; flex-shrink: 0; }
.pf-tag-dot { width: 6px; height: 6px; border-radius: 50%; background: currentColor; flex-shrink: 0; }
${['green','blue','amber','purple','teal','red','indigo','grey'].map(t => `
.pf-tag-${t} { color: var(--t-${t}-fg); background: var(--t-${t}-bg); border-color: var(--t-${t}-bd); }
`).join('')}

.pf-contact-body { display: flex; flex-direction: column; gap: 5px; color: var(--pl-text); }
.pf-contact-line { display: flex; align-items: center; gap: 8px; font-size: 12.5px; color: var(--pl-text-2); text-decoration: none; padding: 4px 0; border-radius: 6px; transition: color .18s; }
a.pf-contact-line:hover { color: var(--pl-mid); }
.pf-contact-icon { width: 22px; height: 22px; border-radius: 7px; background: var(--pl-bg-3); color: var(--pl-muted); display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.pf-contact-text { font-family: ui-monospace, monospace; font-size: 12px; letter-spacing: .01em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; }
.pf-contact-notes { font-style: italic; font-family: Georgia, serif; font-size: 13px; color: var(--pl-text-2); background: var(--pl-bg-3); border-left: 2px solid var(--pl-light-b); padding: 8px 12px; border-radius: 0 8px 8px 0; line-height: 1.5; }
.pf-contact-foot { display: flex; gap: 6px; margin-top: auto; padding-top: .6rem; border-top: 1px solid var(--pl-border); }
...
.pf-empty-card { background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: 16px; padding: 4rem 1.5rem; text-align: center; box-shadow: var(--pl-shadow-1); display: flex; flex-direction: column; align-items: center; gap: 6px; }
.pf-empty-icon { width: 56px; height: 56px; border-radius: 16px; background: var(--pl-light); border: 1px solid var(--pl-light-b); color: var(--pl-mid); display: inline-flex; align-items: center; justify-content: center; margin-bottom: .85rem; }
...
.pf-modal-backdrop { position: fixed; inset: 0; background: color-mix(in oklab, #07140A 70%, transparent); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; padding: 1rem; z-index: 1000; animation: pf-fade .2s ease both; overflow-y: auto; }
...
.pf-type-radio { display: flex; flex-wrap: wrap; gap: 6px; }
.pf-type-radio-btn { display: inline-flex; align-items: center; gap: 6px; font-family: ui-monospace, monospace; font-size: 11px; font-weight: 500; letter-spacing: .04em; text-transform: uppercase; padding: 6px 12px 6px 10px; border-radius: 999px; border: 1px solid var(--pl-border); background: var(--pl-bg-2); color: var(--pl-text-2); cursor: pointer; transition: all .18s ease; }
.pf-type-radio-btn:hover { border-color: var(--pl-border-2); color: var(--pl-text); }
${['green','blue','amber','purple','teal','red','indigo','grey'].map(t => `
.pf-type-radio-${t}.pf-type-radio-active { background: var(--t-${t}-bg); color: var(--t-${t}-fg); border-color: var(--t-${t}-bd); }
`).join('')}
...
@media (prefers-reduced-motion: reduce) { .pf-contacts *, .pf-contacts *::before, .pf-contacts *::after { animation: none !important; transition: none !important; } }
    `}</style>
  )
}

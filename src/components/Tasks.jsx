 ...
 *
 * Functionally identical to the previous Tasks: same Supabase calls,
 * same form fields, same filters, same overdue/due-today/reminder semantics,
 * same linked case handling.
 */
                ...
                return (
                  <tr key={t.id} className={isDone ? 'pf-row-done' : ''}>
                    <td className="pf-cell-title">
                      ...
                      {t.case_id && caseName(t.case_id) && (
                        <div className="pf-task-link">↳ {caseName(t.case_id)}</div>
                      )}
                      {t.notes && <div className="pf-task-notes">{t.notes}</div>}
                    </td>
                    <td className="pf-cell-mono">{propName(t.property_id)}</td>
                    <td>
                      {t.type ? <span className="pf-type-chip">{t.type}</span> : <span className="pf-cell-muted">—</span>}
                    </td>
                    <td>
                      <PriorityBadge level={t.priority} />
                    </td>
                    <td>
                      <DueCell date={t.due_date} isOverdue={isOverdue} isDueToday={isDueToday} />
                    </td>
                    <td>
                      <span className={`pf-cell-mono ${hasReminder ? 'pf-reminder-active' : ''}`}>
                        {t.reminder_date ? fmtDate(t.reminder_date) : '—'}
                      </span>
                    </td>
                    <td>
                ...
                <div className="pf-prio-seg">
                  {['Low', 'Medium', 'High'].map(p => (
                    <button key={p} type="button"
                      className={`pf-prio-seg-btn pf-prio-seg-${p.toLowerCase()} ${form.priority === p ? 'pf-prio-active' : ''}`}
    ...
    return (
      <span className="pf-due-pill pf-due-overdue">
        <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
          <path d="M5.5 1L10.5 10H.5L5.5 1z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/>
          <path d="M5.5 4.5v2.2" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
          <circle cx="5.5" cy="8.2" r=".7" fill="currentColor"/>
        </svg>
        {fmtDate(date)}
      </span>
    )
...
.pf-due-overdue {
  ...
  border-color: var(--pf-red-bd);
  font-weight: 600;
}
.pf-due-today {
  color: var(--pf-amber);
  background: var(--pf-amber-bg);
  border-color: var(--pf-amber-bd);
}
...
.pf-btn-done, .pf-btn-edit, .pf-btn-delete {
  font-family: Georgia, serif;
  font-size: 12px;
  background: transparent;
  border: 1px solid var(--pl-border);
...
.pf-btn-done {
  ...
  background: color-mix(in oklab, var(--pl-light) 50%, transparent);
}
...
.pf-prio-active { box-shadow: 0 1px 2px rgba(20,40,10,.08); font-weight: 500; }
...
@media (prefers-reduced-motion: reduce) {
  .pf-tasks *, .pf-tasks *::before, .pf-tasks *::after {

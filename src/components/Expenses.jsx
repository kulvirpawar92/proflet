...
export default function Expenses({ session, expenses, props, reload }) {
  ...
  function downloadTemplate() {
    ...
    a.href = url; a.download = 'proflet-expenses-template.csv'; a.click()
  ...
  function handleFile(e) {
    ...
    reader.onload = (ev) => {
      ...
      lines.slice(1).forEach((line, idx) => {
        ...
        const rowNum = idx + 2; const rowErrors = []
        ...
        if (rowErrors.length) {
          ...
          rows.push({ property_id: matchedProp.id, property_name: matchedProp.name, type: validTypes.includes(cleanType) ? cleanType : 'Other', amount, date, recurring: isRecurring, notes: notes || null, user_id: session.user.id })
        }
      })
  ...
  async function confirmImport() {
    ...
    setImporting(true)
    const { error } = await supabase.from('expenses').insert(importRows.map(({ property_name, ...row }) => row))
    setImporting(false)
    if (error) { alert('Import failed: ' + error.message) } else {
      alert(`Successfully imported ${importRows.length} expense records!`)
      setShowImport(false); setImportRows([]); setImportErrors([]); reload()
    }
  }
  ...
  function exportCSV() {
    ...
    const rows = expenses
      ...
      .map(e => [propName(e.property_id), e.type || '', e.amount || 0, e.date ? new Date(e.date).toLocaleDateString('en-GB') : '', e.recurring ? 'Monthly' : 'One-off', e.notes || ''])
    const csv = [headers, ...rows].map(row => row.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `proflet-expenses-${new Date().toISOString().slice(0, 10)}.csv`; a.click()
    URL.revokeObjectURL(url)
  }
  ...
  async function save() {
    ...
    setSaving(true)
    const payload = { property_id: form.property_id, type: form.type, amount: parseFloat(form.amount), date: form.date || null, recurring: form.recurring === 'yes', notes: form.notes || null, user_id: session.user.id }
  ...
  async function del(id) {
    ...
    await supabase.from('expenses').delete().eq('id', id); reload()
  }
    ...
    <div className="pf-expenses">
      ...
      <div className="pf-page-head">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          <button className="pf-cta pf-cta-export" onClick={exportCSV}>
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none"><path d="M7 2v8M4 7l3 3 3-3M2 12h10" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
            <span>Export CSV</span>
          </button>
          <button className="pf-cta pf-cta-export" onClick={() => { if (!props.length) { alert('Please add your properties first before importing expense records.'); return }; setImportRows([]); setImportErrors([]); setShowImport(true) }}>
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none"><path d="M7 10V2M4 5l3-3 3 3M2 12h10" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
      ...
      <div className="pf-filters">
        ...
        <div className="pf-filter-meta">
          ...
          {filtersActive && <button className="pf-filter-clear" onClick={() => { setFilterProp('all'); setFilterType('all') }}>Clear</button>}
        </div>
      </div>
      ...
      <div className="pf-table-card">
        <div className="pf-table-scroll">
          <table className="pf-table">
            <thead>
              <tr>
                <th>Property</th><th>Type</th><th className="pf-num">Amount</th>
                <th>Date</th><th>Notes</th><th>Recurring</th><th></th>
              </tr>
            </thead>
            <tbody>
              {!filtered.length && (
                <tr className="pf-empty-row">
                  <td colSpan={7}>
                    <div className="pf-empty">
                      ...
                      {!expenses.length && <button className="pf-cta pf-cta-primary pf-cta-sm" onClick={openAdd} style={{ marginTop: 16 }}><span>+ Add expense</span></button>}
                    </div>
                  </td>
                </tr>
              )}
                ...
                <tr key={e.id}>
                  ...
                  <td>
                    <span className={`pf-badge ${e.recurring ? 'pf-badge-blue' : 'pf-badge-grey'}`}>
                      <span className="pf-badge-dot" />{e.recurring ? 'Monthly' : 'One-off'}
                    </span>
                  </td>
                  <td>
            ...
            <div style={{ padding: '0 1.75rem 1.25rem' }}>
              <div className="pf-import-step">
                ...
                <div className="pf-import-step-body">
                  ...
                  <div className="pf-import-step-desc">Get a CSV template with your property names pre-filled.</div>
                  <button onClick={downloadTemplate} className="pf-cta pf-cta-ghost pf-cta-sm" style={{ marginTop: '10px' }}>↓ Download template</button>
                </div>
              </div>
              <div className="pf-import-step">
                ...
                <div className="pf-import-step-body">
                  ...
                  <div className="pf-import-step-desc">Recurring column accepts: <strong>yes</strong> or <strong>no</strong></div>
                  <input ref={fileRef} type="file" accept=".csv" onChange={handleFile} style={{ display: 'none' }} />
                  <button onClick={() => fileRef.current.click()} className="pf-cta pf-cta-ghost pf-cta-sm" style={{ marginTop: '10px' }}>↑ Choose file</button>
                </div>
              </div>
              ...
              {importRows.length > 0 && (
                <div className="pf-import-preview">
                  <div className="pf-import-preview-title">✓ {importRows.length} record{importRows.length > 1 ? 's' : ''} ready to import</div>
                  <div className="pf-import-table-wrap">
                    <table className="pf-table" style={{ fontSize: '12px' }}>
                      <thead><tr>{['Property','Type','Amount','Date','Recurring','Notes'].map(h => <th key={h}>{h}</th>)}</tr></thead>
                      <tbody>
                        {importRows.slice(0, 5).map((row, i) => (
                          <tr key={i}>
                            ...
                            <td><span className={`pf-badge ${row.recurring ? 'pf-badge-blue' : 'pf-badge-grey'}`}><span className="pf-badge-dot" />{row.recurring ? 'Monthly' : 'One-off'}</span></td>
              ...
              <div className="pf-preview">
                ...
                <div className="pf-preview-line">
                  ...
                  <span className={`pf-badge ${form.recurring === 'yes' ? 'pf-badge-blue' : 'pf-badge-grey'}`}>
                    <span className="pf-badge-dot" />{form.recurring === 'yes' ? 'Monthly' : 'One-off'}
                  </span>
                </div>
              </div>
            )}
...
.pf-metric-accent { position: absolute; top: 0; left: 0; right: 0; height: 2px; }
...
.pf-preview { position: relative; margin-top: 1.25rem; padding: 14px 16px; background: var(--pl-light); border: 1px solid var(--pl-light-b); border-radius: 12px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
...
@media (prefers-reduced-motion: reduce) { .pf-expenses *, .pf-expenses *::before, .pf-expenses *::after { animation: none !important; transition: none !important; } }
    `}</style>
  )
}

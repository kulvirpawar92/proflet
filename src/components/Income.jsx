...
export default function Income({ session, income, props, reload }) {
  ...
  function exportCSV() {
    ...
    const rows = income
      ...
      .map(i => [propName(i.property_id), i.type || '', i.amount || 0,
        i.date ? new Date(i.date).toLocaleDateString('en-GB') : '', i.status || '', i.notes || ''])
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `proflet-income-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
    URL.revokeObjectURL(url)
  }
    ...
    <div className="proflet-income">
      ...
      <div className="pl-inc-page-head">
        ...
        <div className="pl-inc-page-actions">
          ...
          <button onClick={() => {
            if (!props.length) { alert('Please add your properties first before importing income records.'); return }
            setImportRows([]); setImportErrors([]); setShowImport(true)
          }} className="pl-inc-btn pl-inc-btn-ghost">↑ Import CSV</button>
      ...
      <div className="pl-inc-table-card">
        <div className="pl-inc-table-scroll">
          <table className="pl-inc-table">
            <thead>
              <tr>
                {['Property', 'Type', 'Amount', 'Date', 'Notes', 'Status', ''].map(h => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {!filtered.length && (
                <tr>
                  <td colSpan={7} className="pl-inc-empty">
                    ...
                    <div className="pl-inc-empty-text">
                      {income.length === 0 ? 'Add your first income record to start tracking rent and deposits.' : 'Try a different filter or add a new record.'}
                    </div>
                  </td>
                </tr>
              )}
            ...
            <div className="pl-inc-form-body">
              ...
              <div className="pl-import-step">
                ...
                <div className="pl-import-step-body">
                  ...
                  <div className="pl-import-step-desc">Get a CSV template with your property names already filled in. Add your income records then save.</div>
                  <button onClick={downloadTemplate} className="pl-inc-btn pl-inc-btn-ghost" style={{ marginTop: '10px' }}>
                    ↓ Download template
                  </button>
                </div>
              </div>
              ...
              <div className="pl-import-step">
                ...
                <div className="pl-import-step-body">
                  ...
                  <div className="pl-import-step-desc">Select your filled-in CSV file to preview the records before importing.</div>
                  <input ref={fileRef} type="file" accept=".csv" onChange={handleFile} style={{ display: 'none' }} />
                  <button onClick={() => fileRef.current.click()} className="pl-inc-btn pl-inc-btn-ghost" style={{ marginTop: '10px' }}>
                    ↑ Choose file
                  </button>
                </div>
              </div>
              ...
              {importRows.length > 0 && (
                <div className="pl-import-preview">
                  <div className="pl-import-preview-title">
                  ...
                  <div className="pl-import-table-wrap">
                    <table className="pl-inc-table" style={{ fontSize: '12px' }}>
                      <thead>
                        <tr>
                          {['Property', 'Type', 'Amount', 'Date', 'Status', 'Notes'].map(h => <th key={h}>{h}</th>)}
                        </tr>
                      </thead>
                      <tbody>
                        ...
                        {importRows.length > 5 && (
                          <tr><td colSpan={6} style={{ textAlign: 'center', padding: '8px', color: 'var(--pl-muted)', fontSize: '12px' }}>
...
.pl-inc-eyebrow {
  ...
  text-transform: uppercase; color: var(--pl-mid);
...
.pl-inc-btn {
  font-family: Georgia, serif; font-weight: 500; letter-spacing: -0.005em;
  font-size: 13.5px; padding: 9px 18px; border-radius: 999px; border: 0;
...
.pl-inc-metric {
  ...
  border-radius: var(--pl-r-md); padding: 1.1rem 1.3rem 1.15rem; overflow: hidden;
  box-shadow: var(--pl-shadow-1); transition: border-color .2s, box-shadow .2s, transform .2s;
}
...
.pl-inc-select {
  font-family: Georgia, serif; font-size: 13px; background: var(--pl-bg-2);
  border: 1px solid var(--pl-border); color: var(--pl-text);
  padding: 8px 30px 8px 14px; border-radius: 999px; appearance: none; cursor: pointer;
}
...
.pl-inc-status-select {
  font-family: ui-monospace, 'SF Mono', Menlo, monospace; font-size: 11px; font-weight: 500;
  padding: 4px 22px 4px 12px; border-radius: 999px; border: 0; background: transparent;
  color: inherit; cursor: pointer; appearance: none;
  background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 14 14' fill='none'><path d='M3 5.5L7 9.5l4-4' stroke='currentColor' stroke-width='1.6' stroke-linecap='round' stroke-linejoin='round'/></svg>");
...
.pl-inc-modal-bg { position: fixed; inset: 0; background: color-mix(in oklab, #07140A 70%, transparent); backdrop-filter: blur(6px); display: flex; align-items: flex-start; justify-content: center; padding: 2.5rem 1rem; z-index: 1000; overflow-y: auto; animation: pl-inc-fade .25s ease both; }
...
.pl-inc-input { width: 100%; padding: 10px 14px; border-radius: 10px; border: 1px solid var(--pl-border); background: var(--pl-bg); color: var(--pl-text); font-family: Georgia, serif; font-size: 13.5px; outline: 0; transition: border-color .15s, box-shadow .15s; }

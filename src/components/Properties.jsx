    ...
    return (
      <div className="proflet-props">
        ...
        <div className="pl-prop-metric-grid">
          <MetricCard label="Monthly profit" value={money(c.monthlyProfit)} color={c.monthlyProfit >= 0 ? 'var(--pl-mid)' : 'var(--pl-red)'} sub={`${money(c.monthlyProfit * 12)} / yr`} />
          <MetricCard label="Equity" value={money(c.equity)} sub={`LTV ${c.ltv}%`} />
          <MetricCard label="Gross yield" value={c.grossYield + '%'} color={c.grossYield >= 5 ? 'var(--pl-mid)' : 'var(--pl-amber)'} />
          <MetricCard label="Equity release" value={money(c.equityRelease)} color="var(--pl-amber)" sub="At 75% LTV" />
        </div>
        ...
        <div className="pl-prop-tabs">
          {DETAIL_TABS.map(t => (
            <button key={t.id} onClick={() => setDetailTab(t.id)} className={`pl-prop-tab ${detailTab === t.id ? 'pl-prop-tab-active' : ''}`}>{t.label}</button>
          ))}
        </div>
        ...
        {detailTab === 'financials' && (
          <div className="pl-prop-grid-2">
            <div className="pl-prop-card">
              <div className="pl-prop-card-head"><SectionHead>Income</SectionHead><span className="pl-prop-card-meta">{propIncome.length} records</span></div>
              {!propIncome.length ? <div className="pl-prop-empty">No income records</div>
              ...
              <div className="pl-prop-total pl-prop-total-green"><span>Total received</span><span>{money(totalIncome)}</span></div>
            </div>
            <div className="pl-prop-card">
              <div className="pl-prop-card-head"><SectionHead>Expenses</SectionHead><span className="pl-prop-card-meta">{propExpenses.length} records</span></div>
              {!propExpenses.length ? <div className="pl-prop-empty">No expense records</div>
              ...
              <div className="pl-prop-total pl-prop-total-red"><span>Total expenses</span><span>{money(totalExpenses)}</span></div>
            </div>
            <div className="pl-prop-card pl-prop-card-full">
        ...
        {detailTab === 'mortgage' && (
          <div className="pl-prop-grid-2">
            ...
            <div className="pl-prop-card">
              ...
              <DetailRow label="LTV" value={c.ltv + '%'} valueColor={c.ltv > 75 ? 'var(--pl-red)' : c.ltv > 65 ? 'var(--pl-amber)' : 'var(--pl-mid)'} />
          ...
          <div className="pl-prop-grid-2">
            <div className="pl-prop-card">
              ...
              {detailProp.tenancy_end && (() => {
                ...
                return <div className={`pl-prop-callout pl-prop-callout-${tone}`}>{d <= 0 ? 'Tenancy has ended' : `${d} days until tenancy ends`}</div>
              })()}
            </div>
            <div className="pl-prop-card">
              ...
              {detailProp.insurance_renewal && (() => {
                ...
                return <div className={`pl-prop-callout pl-prop-callout-${tone}`}>{d <= 0 ? 'Insurance has expired!' : `${d} days until renewal`}</div>
              })()}
        ...
        <div className="pl-prop-modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="pl-prop-form-body">
            {formTab === 'basic' && (
              <div className="pl-prop-form-grid">
                <Field full label="Property name *"><input type="text" value={form.name || ''} onChange={f('name')} placeholder="e.g. 35 Lincoln Way" className="pl-prop-input" /></Field>
                <Field full label="Full address"><input type="text" value={form.address || ''} onChange={f('address')} className="pl-prop-input" /></Field>
                <Field label="Postcode"><input type="text" value={form.postcode || ''} onChange={f('postcode')} className="pl-prop-input" /></Field>
                <Field label="Type">
                ...
                <Field label="Bedrooms"><input type="number" value={form.bedrooms || ''} onChange={f('bedrooms')} className="pl-prop-input" /></Field>
                <Field label="Bathrooms"><input type="number" value={form.bathrooms || ''} onChange={f('bathrooms')} className="pl-prop-input" /></Field>
                <Field label="Purchase price (£)"><input type="number" value={form.purchase_price || ''} onChange={f('purchase_price')} className="pl-prop-input" /></Field>
                <Field label="Current value (£)"><input type="number" value={form.current_value || ''} onChange={f('current_value')} className="pl-prop-input" /></Field>
                <Field label="Purchase date"><input type="date" value={form.purchase_date || ''} onChange={f('purchase_date')} className="pl-prop-input" /></Field>
                <Field label="Status">
                ...
                <Field full label="Notes"><textarea value={form.notes || ''} onChange={f('notes')} rows={2} className="pl-prop-input pl-prop-textarea" /></Field>
              </div>
            )}
            {formTab === 'tenancy' && (
              <div className="pl-prop-form-grid">
                <Field label="Monthly rent (£)"><input type="number" value={form.monthly_rent || ''} onChange={f('monthly_rent')} className="pl-prop-input" /></Field>
                ...
                <Field label="Tenancy start"><input type="date" value={form.tenancy_start || ''} onChange={f('tenancy_start')} className="pl-prop-input" /></Field>
                <Field label="Tenancy end"><input type="date" value={form.tenancy_end || ''} onChange={f('tenancy_end')} className="pl-prop-input" /></Field>
              </div>
            )}
            {formTab === 'mortgage' && (
              <div className="pl-prop-form-grid">
                <Field label="Lender"><input type="text" value={form.mortgage_lender || ''} onChange={f('mortgage_lender')} className="pl-prop-input" /></Field>
                <Field label="Product name"><input type="text" value={form.mortgage_product || ''} onChange={f('mortgage_product')} className="pl-prop-input" /></Field>
                <Field label="Balance (£)"><input type="number" value={form.mortgage_balance || ''} onChange={f('mortgage_balance')} className="pl-prop-input" /></Field>
                <Field label="Monthly payment (£)"><input type="number" value={form.mortgage_payment || ''} onChange={f('mortgage_payment')} className="pl-prop-input" /></Field>
                <Field label="Interest rate (%)"><input type="number" step="0.01" value={form.interest_rate || ''} onChange={f('interest_rate')} className="pl-prop-input" /></Field>
                <Field label="Fixed term ends"><input type="date" value={form.fixed_term_end || ''} onChange={f('fixed_term_end')} className="pl-prop-input" /></Field>
                <Field label="Remortgage date"><input type="date" value={form.remortgage_date || ''} onChange={f('remortgage_date')} className="pl-prop-input" /></Field>
              </div>
            )}
            {formTab === 'insurance' && (
              <div className="pl-prop-form-grid">
                <Field label="Insurance provider"><input type="text" value={form.insurance_provider || ''} onChange={f('insurance_provider')} className="pl-prop-input" /></Field>
                <Field label="Policy number"><input type="text" value={form.policy_number || ''} onChange={f('policy_number')} className="pl-prop-input" /></Field>
                <Field label="Renewal date"><input type="date" value={form.insurance_renewal || ''} onChange={f('insurance_renewal')} className="pl-prop-input" /></Field>
              </div>
            )}
          </div>
          <div className="pl-prop-modal-foot">
            ...
            <div className="pl-prop-modal-actions">
              ...
              {formTab !== 'basic' && <GhostButton onClick={() => setFormTab(FORM_TABS[idx - 1].id)}>← Back</GhostButton>}
              {formTab !== 'insurance'
    ...
    <div className="proflet-props">
      ...
      {!filtered.length && (
        <div className="pl-prop-empty-state">
          ...
          <div className="pl-prop-empty-text">
            {props.length === 0 ? 'Add your first property to start tracking rent, mortgage and equity.' : 'Try a different filter or add a new property.'}
          </div>
      ...
      <div className="pl-prop-card-grid">
        {filtered.map(p => {
          ...
          return (
            <article key={p.id} onClick={() => openDetail(p)} className="pl-prop-card pl-prop-card-clickable">
...
.pl-prop-eyebrow { font-family: ui-monospace, 'SF Mono', Menlo, monospace; font-size: 11px; font-weight: 500; letter-spacing: .14em; text-transform: uppercase; color: var(--pl-mid); display: inline-flex; align-items: center; gap: 8px; }
.pl-prop-eyebrow::before { content: ''; width: 22px; height: 1px; background: currentColor; opacity: .5; display: inline-block; }
.pl-prop-sec-head { font-family: ui-monospace, 'SF Mono', Menlo, monospace; font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: .12em; color: var(--pl-muted); margin: 1.25rem 0 .6rem; }
.pl-prop-sec-head:first-child { margin-top: 0; }

.pl-prop-page-head { display: flex; align-items: flex-end; justify-content: space-between; gap: 16px; flex-wrap: wrap; margin-bottom: 1.5rem; }
.pl-prop-page-title { font-family: Georgia, serif; font-weight: 500; font-size: clamp(28px, 3.4vw, 38px); letter-spacing: -0.02em; margin: .35rem 0 .25rem; color: var(--pl-text); line-height: 1.1; }
...
.pl-prop-btn { font-family: Georgia, serif; font-weight: 500; letter-spacing: -0.005em; border-radius: 999px; border: 0; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; white-space: nowrap; transition: transform .18s ease, box-shadow .18s ease, background .18s ease, color .18s ease, border-color .18s ease; }
...
.pl-prop-btn-primary { color: white; background: linear-gradient(180deg, var(--pl-bright) 0%, var(--pl-mid) 100%); box-shadow: 0 1px 0 rgba(255,255,255,.25) inset, 0 -1px 0 rgba(0,0,0,.15) inset, 0 8px 22px -8px rgba(59,109,17,.55), 0 2px 4px rgba(20,40,10,.18); }
.pl-prop-btn-primary:hover { transform: translateY(-1px); }
.pl-prop-btn-ghost { background: var(--pl-bg-2); color: var(--pl-text-2); border: 1px solid var(--pl-border); }
...
.pl-prop-select { font-family: Georgia, serif; font-size: 13px; background: var(--pl-bg-2); border: 1px solid var(--pl-border); color: var(--pl-text); padding: 8px 30px 8px 14px; border-radius: 999px; appearance: none; -webkit-appearance: none; cursor: pointer; transition: border-color .15s; }
...
.pl-badge { display: inline-flex; align-items: center; font-size: 11px; font-weight: 500; font-family: ui-monospace, 'SF Mono', Menlo, monospace; letter-spacing: .04em; padding: 3px 10px; border-radius: 999px; white-space: nowrap; border: 1px solid transparent; }
...
.pl-prop-empty-state { text-align: center; padding: 4rem 1.5rem; background: var(--pl-bg-2); border: 1px dashed var(--pl-border-2); border-radius: var(--pl-r-lg); }
.pl-prop-empty-mark { width: 56px; height: 56px; border-radius: 16px; background: var(--pl-light); border: 1px solid var(--pl-light-b); color: var(--pl-mid); display: inline-flex; align-items: center; justify-content: center; margin-bottom: 1rem; }
...
.pl-prop-card-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 14px; }
.pl-prop-card { position: relative; background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: var(--pl-r-lg); padding: 1.4rem 1.5rem 1.25rem; overflow: hidden; transition: transform .2s ease, box-shadow .2s ease, border-color .2s ease; }
.pl-prop-card-clickable { cursor: pointer; }
.pl-prop-card-glow { position: absolute; inset: 0; background: radial-gradient(circle at 0% 0%, color-mix(in oklab, var(--pl-bright) 8%, transparent), transparent 60%); opacity: 0; transition: opacity .25s ease; pointer-events: none; }
.pl-prop-card-clickable:hover { transform: translateY(-3px); box-shadow: var(--pl-shadow-2); border-color: var(--pl-light-b); }
...
.pl-prop-thumb { width: 36px; height: 36px; border-radius: 10px; background: var(--pl-light); color: var(--pl-mid); border: 1px solid var(--pl-light-b); display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.pl-prop-card-stats { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; margin-bottom: 1rem; position: relative; }
.pl-prop-card-stat { background: var(--pl-bg-3); border-radius: 8px; padding: 8px 10px; border: 1px solid transparent; }
...
.pl-prop-card-ltv-track { height: 5px; background: var(--pl-bg-3); border-radius: 999px; overflow: hidden; }
...
.pl-prop-card-foot-edit, .pl-prop-card-foot-del { font-family: Georgia, serif; font-size: 12.5px; padding: 6px 14px; border-radius: 999px; cursor: pointer; transition: background .15s, color .15s, border-color .15s; }
.pl-prop-card-foot-edit { flex: 1; background: var(--pl-light); color: var(--pl-mid); border: 1px solid var(--pl-light-b); font-weight: 500; }
.pl-prop-card-foot-edit:hover { background: color-mix(in oklab, var(--pl-light) 75%, var(--pl-light-b)); }
.pl-prop-card-foot-del { background: transparent; color: var(--pl-muted); border: 1px solid var(--pl-border); }
.pl-prop-card-foot-del:hover { color: var(--pl-red); border-color: var(--pl-red); background: var(--pl-red-bg); }
.pl-prop-card-foot-go { margin-left: auto; display: inline-flex; align-items: center; gap: 4px; font-family: ui-monospace, monospace; font-size: 11px; text-transform: uppercase; letter-spacing: .08em; color: var(--pl-mid); opacity: .6; transition: opacity .15s, transform .15s; }
.pl-prop-card-clickable:hover .pl-prop-card-foot-go { opacity: 1; transform: translateX(2px); }

.pl-prop-detail-head { display: grid; grid-template-columns: auto 1fr auto; gap: 16px; align-items: center; margin-bottom: 1.25rem; padding: 1rem 1.25rem; background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: var(--pl-r-lg); position: relative; overflow: hidden; }
.pl-prop-detail-head::before { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse 60% 80% at 0% 0%, color-mix(in oklab, var(--pl-bright) 10%, transparent), transparent 60%); pointer-events: none; }
.pl-prop-back { position: relative; display: inline-flex; align-items: center; gap: 6px; font-family: Georgia, serif; font-size: 12.5px; padding: 7px 14px; border-radius: 999px; background: var(--pl-bg-3); color: var(--pl-text-2); border: 1px solid var(--pl-border); cursor: pointer; }
.pl-prop-back:hover { color: var(--pl-text); border-color: var(--pl-border-2); }
.pl-prop-detail-title-wrap { position: relative; min-width: 0; }
.pl-prop-detail-title { font-family: Georgia, serif; font-weight: 500; font-size: clamp(22px, 2.6vw, 28px); letter-spacing: -0.015em; color: var(--pl-text); margin: .25rem 0; line-height: 1.15; }
...
.pl-prop-metric-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 1.25rem; }
.pl-prop-metric { background: var(--pl-bg-2); border: 1px solid var(--pl-border); border-radius: var(--pl-r-md); padding: 1rem 1.15rem; position: relative; overflow: hidden; transition: border-color .2s, box-shadow .2s; }
.pl-prop-metric::after { content: ''; position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, var(--pl-bright), var(--pl-mid)); opacity: .85; }
...
.pl-prop-alert { display: inline-flex; align-items: center; gap: 8px; padding: 7px 14px; border-radius: 999px; font-size: 12px; font-family: Georgia, serif; border: 1px solid transparent; }
...
.pl-prop-tabs { display: flex; gap: 4px; border-bottom: 1px solid var(--pl-border); margin-bottom: 1.25rem; overflow-x: auto; }
.pl-prop-tab { font-family: Georgia, serif; font-size: 13.5px; padding: 10px 18px; background: transparent; border: 0; color: var(--pl-muted); cursor: pointer; border-bottom: 2px solid transparent; margin-bottom: -1px; transition: color .15s, border-color .15s; display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; }
.pl-prop-tab:hover { color: var(--pl-text-2); }
.pl-prop-tab-active { color: var(--pl-mid); border-color: var(--pl-mid); font-weight: 500; }
.pl-prop-tab-num { display: inline-flex; align-items: center; justify-content: center; width: 20px; height: 20px; border-radius: 50%; font-family: ui-monospace, monospace; font-size: 10px; background: var(--pl-bg-3); color: var(--pl-muted); border: 1px solid var(--pl-border); }
...
.pl-prop-detail-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid var(--pl-border); font-size: 13px; }
...
.pl-prop-line { display: flex; justify-content: space-between; padding: 7px 0; border-bottom: 1px solid var(--pl-border); font-size: 13px; }
...
.pl-prop-total { margin-top: .85rem; padding: .85rem 1rem; border-radius: 10px; display: flex; justify-content: space-between; font-size: 13px; font-weight: 500; border: 1px solid transparent; }
...
.pl-prop-equity-card { margin-top: 1rem; padding: 1.1rem 1.25rem; background: var(--pl-light); border: 1px solid var(--pl-light-b); border-radius: var(--pl-r-md); }
...
.pl-prop-callout { margin-top: 1rem; padding: .85rem 1rem; border-radius: 10px; font-size: 12.5px; font-weight: 500; border: 1px solid transparent; }
...
.pl-prop-modal-bg { position: fixed; inset: 0; background: color-mix(in oklab, #07140A 70%, transparent); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); display: flex; align-items: flex-start; justify-content: center; padding: 2.5rem 1rem; z-index: 1000; overflow-y: auto; animation: pl-prop-fade .25s ease both; }
@keyframes pl-prop-fade { from { opacity: 0 } to { opacity: 1 } }
.pl-prop-modal { position: relative; width: 100%; max-width: 600px; background: var(--pl-bg-2); border: 1px solid var(--pl-border-2); border-radius: 22px; box-shadow: var(--pl-shadow-3); overflow: hidden; animation: pl-prop-pop .35s cubic-bezier(.2,.7,.2,1) both; }
@keyframes pl-prop-pop { from { opacity: 0; transform: translateY(12px) scale(.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
.pl-prop-modal-glow { position: absolute; inset: 0; background: radial-gradient(ellipse 80% 40% at 50% 0%, color-mix(in oklab, var(--pl-bright) 18%, transparent), transparent 70%); pointer-events: none; }
.pl-prop-modal-head { position: relative; display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 1.5rem 1.75rem 1rem; }
...
.pl-prop-modal-close { background: var(--pl-bg-3); border: 1px solid var(--pl-border); border-radius: 999px; width: 32px; height: 32px; font-size: 20px; line-height: 1; color: var(--pl-muted); cursor: pointer; transition: color .15s, border-color .15s; }
...
.pl-prop-input { width: 100%; padding: 10px 14px; border-radius: 10px; border: 1px solid var(--pl-border); background: var(--pl-bg); color: var(--pl-text); font-family: Georgia, serif; font-size: 13.5px; outline: 0; transition: border-color .15s, box-shadow .15s; colorScheme: dark; }
...
.pl-prop-modal-foot { display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.75rem 1.25rem; border-top: 1px solid var(--pl-border); background: var(--pl-bg-3); }
.pl-prop-step-dots { display: inline-flex; gap: 6px; }
.pl-prop-step-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--pl-border); transition: background .2s; }

...
export default function Cases({ session, cases, props, contacts, reload }) {
  ...
  async function save() {
    ...
    setSaving(true)
    const payload = { title: form.title, type: form.type || null, property_id: form.property_id || null, priority: form.priority, status: form.status, target_date: form.target_date || null, contact_id: form.contact_id || null, notes: form.notes || null, user_id: session.user.id }
    if (editing) await supabase.from('cases').update(payload).eq('id', editing)
  ...
  async function del(id) {
    ...
    await supabase.from('cases').delete().eq('id', id); reload()
  }

  async function updateStatus(id, status) {
    await supabase.from('cases').update({ status }).eq('id', id); reload()
  }
    ...
    <div className="pf-cases">
      ...
      <div className="pf-filters">
        ...
        <div className="pf-filter-meta">
          ...
          {filtersActive && <button className="pf-filter-clear" onClick={() => { setFilterStatus('open'); setFilterPriority('all'); setFilterProp('all'); setFilterType('all') }}>Clear</button>}
        </div>
      </div>
          ...
          <table className="pf-table">
            <thead>
              <tr>
                ...
                <th>Status</th><th>Due</th><th>Contact</th><th></th>
              </tr>
            </thead>
            <tbody>
              {!filtered.length && (
                <tr className="pf-empty-row">
                  <td colSpan={8}>
                    <div className="pf-empty">
                      ...
                      {!cases.length && <button className="pf-cta pf-cta-primary pf-cta-sm" onClick={openAdd} style={{ marginTop: 16 }}><span>+ New case</span></button>}
                    </div>
                  </td>
                </tr>
              )}
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td>{c.type ? <span className="pf-chip">{c.type}</span> : <span className="pf-cell-mono">—</span>}</td>
                    <td>
                      <span className={`pf-badge pf-badge-${PRIORITY_KEY[c.priority] || 'grey'}`}>
                        <span className="pf-badge-dot" />{c.priority || '—'}
                      </span>
                    </td>
                    <td>
                      <div className={`pf-status-select pf-badge-${STATUS_KEY[c.status] || 'grey'}`}>
                        <span className="pf-badge-dot" />
                        <select value={c.status || 'New'} onChange={e => updateStatus(c.id, e.target.value)} aria-label="Update status">
                    ...
                    <td>
                      {c.target_date ? (
                        <span className={`pf-date ${isOverdue ? 'pf-date-overdue' : ''}`}>
                          {isOverdue && <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true"><path d="M6 1.5l5 9H1l5-9zM6 5v2.5M6 8.5v.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>}
          ...
          <div className="pf-modal">
            ...
            <div className="pf-form-grid">
              <Field label="Title" required full>
                <input type='text' value={form.title} onChange={e => setField('title', e.target.value)} placeholder='e.g. Boiler repair needed' className="pf-input" autoFocus />
              </Field>
              ...
              <Field label="Priority">
                <div className="pf-segmented" role="group" aria-label="Priority">
                  {['Low', 'Medium', 'High'].map(o => (
                    <button type="button" key={o} onClick={() => setField('priority', o)} className={`pf-seg pf-seg-${PRIORITY_KEY[o]} ${form.priority === o ? 'pf-seg-active' : ''}`}>
                      <span className="pf-badge-dot" />{o}
                    </button>
                  ))}
                </div>
              </Field>
              ...
              <Field label="Notes" full>
                <textarea value={form.notes} onChange={e => setField('notes', e.target.value)} rows={3} placeholder='Describe the issue…' className="pf-input pf-textarea" />
              </Field>
            </div>
...
.pf-title-main { font-family: Georgia, serif; font-weight: 500; color: var(--pl-text); letter-spacing: -0.005em; }
.pf-title-notes { font-size: 11.5px; color: var(--pl-muted); margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 240px; font-style: italic; }
.pf-cell-mono { font-family: ui-monospace, monospace; font-size: 12px; color: var(--pl-text-2); }
.pf-chip { display: inline-flex; align-items: center; font-family: ui-monospace, monospace; font-size: 11px; font-weight: 500; padding: 3px 10px; border-radius: 999px; letter-spacing: .02em; white-space: nowrap; background: var(--pl-bg-3); color: var(--pl-text-2); border: 1px solid var(--pl-border); }
.pf-date { display: inline-flex; align-items: center; gap: 6px; font-family: ui-monospace, monospace; font-size: 12px; color: var(--pl-text-2); }
.pf-date-overdue { color: var(--pf-red); font-weight: 600; background: var(--pf-red-bg); border: 1px solid var(--pf-red-bd); padding: 2px 9px 2px 7px; border-radius: 999px; }
...
.pf-status-select { position: relative; display: inline-flex; align-items: center; gap: 6px; padding: 3px 24px 3px 8px; border-radius: 999px; border: 1px solid transparent; font-family: ui-monospace, monospace; font-size: 11px; font-weight: 500; letter-spacing: .02em; cursor: pointer; transition: filter .15s ease; }
.pf-status-select:hover { filter: brightness(.9); }
.pf-status-select select { appearance: none; -webkit-appearance: none; background: transparent; border: 0; outline: 0; font: inherit; color: inherit; cursor: pointer; padding: 0 2px 0 0; letter-spacing: .02em; }
.pf-status-select .pf-caret { right: 8px; color: currentColor; opacity: .65; }
...
.pf-modal { position: relative; background: var(--pl-bg-2); border: 1px solid var(--pl-border-2); border-radius: 22px; padding: 1.75rem; width: 100%; max-width: 560px; max-height: calc(100vh - 2rem); overflow-y: auto; box-shadow: var(--pl-shadow-3); animation: pf-pop .25s ease both; }
...
.pf-segmented { display: inline-flex; background: var(--pl-bg); border: 1px solid var(--pl-border); border-radius: 10px; padding: 3px; gap: 2px; width: 100%; }
.pf-seg { flex: 1; display: inline-flex; align-items: center; justify-content: center; gap: 6px; font-family: ui-monospace, monospace; font-size: 12px; letter-spacing: .02em; padding: 6px 8px; border-radius: 7px; cursor: pointer; background: transparent; border: 0; color: var(--pl-muted); transition: background .18s, color .18s; }
...
@media (prefers-reduced-motion: reduce) { .pf-cases *, .pf-cases *::before, .pf-cases *::after { animation: none !important; transition: none !important; } }
    `}</style>
  )
}
